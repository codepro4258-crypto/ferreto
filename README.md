...
`https://docs.google.com/spreadsheets/d/1C8lKTDgcNd_6RdZYmtwC-dVlDMCvqx56QsgSzL9iMcE/edit?usp=sharing`
...
function doGet(e) {
  ...
  if (action === 'load') {
    return ContentService
      .createTextOutput(JSON.stringify({ sheetId: sheetId || null, data }))
...
function doPost(e) {
  ...
  if (body.action === 'save' && body.data) {
    ...
    return ContentService
      .createTextOutput(JSON.stringify({ ok: true, sheetId: targetSheetId || null }))
      .setMimeType(ContentService.MimeType.JSON);
  }
...
window.FERRETTO_GOOGLE_SHEETS_URL = "https://docs.google.com/spreadsheets/d/1C8lKTDgcNd_6RdZYmtwC-dVlDMCvqx56QsgSzL9iMcE/edit?usp=sharing";
</script>
```

6. Reload the app. Data edits (users/projects/etc.) will now sync through Google and can be accessed worldwide.

> The app automatically extracts and sends this sheet ID to your Apps Script endpoint: `1C8lKTDgcNd_6RdZYmtwC-dVlDMCvqx56QsgSzL9iMcE`.
